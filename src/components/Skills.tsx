  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.h2
          className="text-3xl md:text-4xl mb-8 text-center text-[var(--light)]"
      ...
      <div className="w-full overflow-hidden relative mb-16 md:mb-20" style={{ maskImage: 'linear-gradient(to right, transparent, black 10%, black 90%, transparent)' }}>
        <motion.div
          className="flex items-center bg-black/20 rounded-lg p-4"
          animate={{ x: ['0%', '-100%'] }}
        ...
        >
          {duplicatedTools.map((tool, index) => (
            <div key={index} className="flex-shrink-0 mx-8 flex flex-col items-center justify-center h-28 w-40">
              ...
              <span className="text-sm text-white mt-2 text-center">{tool.name}</span>
            </div>
          ))}
        </motion.div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.h3
          className="text-2xl md:text-3xl mb-12 text-center text-[var(--light)]"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          {methods.map((method) => (
            <motion.div
              key={method}
              className="px-4 py-2 rounded-full text-sm font-medium cursor-pointer transition-all duration-300 border border-gray-600/50 hover:border-blue-400/50"
              style={{
