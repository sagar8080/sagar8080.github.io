...
flowchart TB
  ...
  classDef cost fill:#fce8e0,stroke:#b45a3c,color:#6A3320
  classDef win fill:#e6e9d6,stroke:#6b7a4e,color:#3F4A24

  subgraph A["Polyglot persistence"]
    direction LR
    A1[("document catalog")]:::plain
    A2[("graph lineage")]:::plain
    A3[("KV memory")]:::plain
    A4[("SQL policy")]:::plain
    AA["4 processes · 3 query languages · 4 backups · joins are orchestration"]:::cost
  end

  subgraph B["Single multi-model embedded"]
    direction LR
    B1[("documents · graph · KV · indexes")]:::accent
    BB["1 process · 1 query language · 1 backup file · joins at decision time"]:::win
  end
```
